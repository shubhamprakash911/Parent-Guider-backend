...
def history():
    ...
    if history:
        return jsonify(
            {
                "message": "Chat history has been retrieved successfully.",
        ...
        return jsonify(
            {
                "message": "No Chat history available, Please login first",
                "success": False,
            }
        )
...
def login():
    ...
    if user and bcrypt.checkpw(password.encode("utf-8"), user["password"]):
        ...
        return jsonify(
            {"message": "Login successful!", "token": token, "email": user["email"]}
        )
